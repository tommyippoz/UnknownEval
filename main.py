...
from utils.Estimator import PYODEstimator
...
def data_analysis(classifier, tr_x, tr_y, te_x, te_y, unk_x, unk_y):
    ...
    print("Accuracy/MCC = " + '{0:.4f}'.format(accuracy) + "/" + '{0:.4f}'.format(mcc) +
          ...
          " (" + '{0:.4f}'.format(100*len(unk_y)/len(te_y)) + "%) of unknowns [" + classifier_name + "]")
...
if __name__ == '__main__':
    ...
    with open(SCORES_FILENAME, "w") as myfile:
        # Print Header
        myfile.write("datasetName,attLabel,classifierName,trainTime,testTime,tp,tn,fp,fn,acc,rec,mcc,tp_u,fn_u,rec_unk\n")
